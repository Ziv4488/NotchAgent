...
struct IslandShell: View {
    ...
    /// 岛的外沿画不画。**只有展开态画。**
    ///
    /// 08-04 的规矩是「idle 不画，其余都画」，理由是外沿给**从菜单栏长出来的
    /// 那部分**用 —— 岛探到桌面上就需要一条边把自己和壁纸分开。
    ///
    /// **08-12 用户按实机否掉了后半句**：running 和 notice 都是**没人在看的时候
    /// 自己冒出来的**形态，描边加投影让它在壁纸上"浮"起来，一眼就是个悬浮控件，
    /// 不是刘海的延伸。原话是「工作时和通知态的边框有问题，这些应该都保持
    /// idle 态常驻无边框的状态」。
    ///
    /// 展开态留着：那是用户自己点开的，岛这时候本来就是一个面板，
    /// 一条边把它和底下的窗口分开是有用的。
    private var showsEdges: Bool { model.state == .expanded }
