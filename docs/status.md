...
更新于 2026-08-04（`2ed59a9`）。**这页只写"当前状态"，细节不搬**——
...
- **外沿与圆角等实测**：08-04 按用户给的 macOS 26 窗口截图改了下角（12→16）和
  外沿三层（内 1pt 白 20% + 外 0.5pt 黑 + 阴影 0.7/18pt/下偏 6）；当天晚上又按
  用户看实机的反馈收了两处 —— **顶边不描线**（`NotchShape.closesTop`）、
  **idle 态整套外沿不上**（回到 08-02 那版的纯黑一块）。参数都是量出来的，
  但**浓淡好不好看只能人眼判** —— §1.7、§1.5b（浅/深两张壁纸）、§1.5c（看上沿）等复测
- **拖拽热区宽度**：用户 08-04 定了「内 4pt 外 4pt」，跨在岛的边线两侧，
  **但要等外沿定稿之后再动**（形状还没最终确认）。光标那一半便宜（跟踪区不看命中
  ...
  的回归风险
- **观感批改**：08-04 做了 13.13（滑动 + 不透光）、13.13b（芯片改胶囊）、
  10.10e（拆掉结束态底下那条没用的输入框），都等复测。用户说「还有其他观感需要调」，
  剩下的要攒一批一起改
