...
更新于 2026-08-06（终端主题之后）。**这页只写"当前状态"，细节不搬**——
...
- **`~/Applications/AXProbe.app` 欠着一笔删除。** 08-06 为跑 AX 探针（spec 11.4）
  ...
  **08-07 起它还多了一个屏幕录制授权**（为跑镜像方案的探针），删的时候
  两个开关都要撤
- **收起岛时那个 app 窗口怎么办，还没定。** 08-07 用户报「用台前调度，收起时
  窗口在左边打开的感觉很怪」——现在的做法是 `hide()`（等价 ⌘H，spec 第 279 行
  写死的），台前调度下 ⌘H 就是把 app 甩进左边那条。用户选了「挪出屏幕」，
  但探针量下来**挪不出去**：往下钳在 y=902（留 80px）、往右钳在 x=1472
  （留 40px）、往上直接弹回 y=33。设回原 frame 是完好的。剩下的选项是
  「收起就还原到原位」／「最小化到 Dock」／「留在原地不动」，等镜像方案探完一起定
- **在评估「镜像」这条路。** 用户 08-07 追问「为什么融不进岛」。根因是终端是
  我们自己进程里的一个 view，而第三方窗口是**另一个进程的独立窗口** ——
  macOS 不给跨进程 reparent，标题栏／圆角／阴影一概改不了，能做的只有挪、缩、藏。
  真要做到和终端一样只有一条路：ScreenCaptureKit 逐帧抓 + `CGEventPostToPid`
  转发输入。探针在跑，三条生死线是「窗口藏起来之后还抓不抓得到」
  「输入转发得回去吗」「帧率延迟 CPU 够不够」
