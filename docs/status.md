...
### 08-12 收的两处观感（用户都复测过了）

- **外沿只剩展开态**。原来是「idle 不画、其余都画」，用户报「工作时和通知态的边框
  有问题，这些应该都保持 idle 态常驻无边框的状态」—— running 和 notice 都是没人在
  看的时候岛自己冒出来的，一描边就成了浮在壁纸上的控件。`showsEdges` 改成
  `state == .expanded`，`IslandPixelTests` 一正一反两条钉着
- **菜单左对齐、退出不带图标**。macOS 26 会给 `terminate:` 这种标准动作自动配
  SF Symbol，而一个带图标的项会把同一段里所有项的标题一起往右推 ——「偏好设置…」
  就是这么被顶缩进的。退出改走自定义 selector 就不匹配那张表了
