...
更新于 2026-08-04（`c20de99`）。**这页只写"当前状态"，细节不搬**——
...
- **观感批改**：13.13「过渡希望稍微丝滑一点」还没动。用户说「还有其他观感需要调」，
  要攒一批一起改，所以在他列完之前不碰动画
- **拖拽热区宽度**：现在 8pt、全落在岛里面（再宽会撞上 ✕）。系统窗口的可抓边是跨在
  边线两侧的。等第六版光标实测确认稳了再问要不要往岛外放几 pt ——
  光标那一半便宜（跟踪区不看命中测试），拖拽那一半得动 `NotchHostingView.hitTest`，
  有 §2.2b「点击被透明画布吞掉」的回归风险
