...
日期：2026-07-30
状态：第 1、2 阶段已实现（2026-08-04）；第 3 阶段贴附未开工。当前进度见 `docs/status.md`。
本文里若有和代码对不上的地方，各小节旁边有批注写明是哪一版改的、为什么。
...
| `expanded` | 宽默认 560pt、内容区默认 **342pt**，**两者都可拖拽调整并记住**；高 = 菜单栏高 + tab 条 + 内容区 + 输入框 | tab 条 + 内容区 + 输入框 | 用户点击岛 |

> **内容区那个默认值 2026-08-02 从 320 改成 342，岛的高度一点没变。** 用量条
> （22pt）8/1 拆掉之后，它的高度一直还留在 `IslandMetrics.expandedChromeHeight`
> 里，多出来的 22pt 被内容区（`maxHeight: .infinity`）默默吃掉 —— 也就是说内容区
> **实际**一直是 342，只有常量在说 320。现在把这 22pt 从 chrome 挪回内容区，
> 两个数各自说真话，总高不变。`UserDefaults` 里存过的旧口径高度读的时候补一次
> （`Preferences.usageBarReclaim`），否则已经拖过尺寸的岛会当场矮 22pt。
...
单面板 + 标签页。顶部状态带（避开物理刘海，左右分置）→ tab 条 → 内容区。
（原来内容区下面还有一条输入框，2026-08-04 拆了，见 5.2 的批注。）

一次只显示一个 tab 的内容：

- **CLI tab**：内容区是 SwiftTerm 终端视图。**会话活着时下面没有输入框** —— 键盘直接归终端（理由见 5.2）；会话结束后输入框回来
- **app tab**：内容区和输入框整体不绘制，只保留状态带 + tab 条，第三方 app 的真实窗口紧贴 tab 条下方、同宽、圆角对齐

tab 条末尾是 `＋`，进入新建流程。

> ### 用量条：**2026-08-01 整块拆掉了，这一节作废**
>
> 原设计是输入框上方一行 22pt 的常驻条：左边 `ctx` / `5h` / `周` 三条额度，
> 右边子代理数与权限模式芯片。做出来之后用户的原话是「这部份不要了，没有重复的
> 必要」—— 会话活着的时候终端里 Claude Code 自己那条 statusline 就写着上下文和
> 模式，岛再抄一遍是同一份信息占两行；而会话不在跑的时候那三个数也没意义。
>
> 一起去掉的还有中断按钮：中断已经归 `Esc`，它直接进 PTY，比按一个我们画的按钮
> 更接近真终端。`UsageProbe.swift` 与它的测试一并删除，`SessionUsage` 只剩
> `mode` 与 `subagents`（**当前没有任何界面显示它们**，见下方 5.2b 的批注）。
>
> 输入框只服务于两种场合：新建流程，以及会话已结束的 tab。那时候它的发送键就是
> 发送键，没有「变成停止键」这一说 —— 没有在跑的东西可停。
>
> **2026-08-04 更新：那条输入框整个拆了。** 新建流程走的是 `NewTaskForm` 自己的
> 指令框，跟它无关；而「会话已结束」那半句是错的 —— 会话结束意味着 PTY 已经死了，
> 回车写进去什么都不会发生。用户看实机的原话是「没有任何作用的，整合成一个内框」。
> 展开态底下现在只有内容区那一张卡。要接着聊按「继续上次会话」（`--resume`）。
> 岛的总高不变，那 44pt 由内容区吃掉（`IslandConstants.retiredInputBarHeight`）。
...
> **2026-08-01：这一整节作废。** 上下文 / 5 小时 / 周三项连同用量条一起拆了
> （理由见 3.2 那条批注），`UsageProbe.swift` 已删除。下面这些留作记录 ——
> 万一以后要把哪一项加回来，口径和坑都在这儿。
>
> 现在还活着的只有 `mode` 与 `subagents`：`StatusFeed` 照旧从 hook 里算出来填进
> `SessionUsage`，但**没有任何界面显示它们**。
...
| 5 小时 / 周 | ~~`~/.claude.json` 的 `cachedUsageUtilization`~~ | **2026-08-01 删除** |
...
v1：任务是 app 的子进程。退出 app 时若有任务在跑，先弹确认对话框；确认后终止全部子进程。**关单个 tab 同理** —— 会话还活着就先弹框（2026-08-02 加的，那一下点下去跑到一半的那一轮就丢了，和退 app 是同一件事的两个尺度）。会话记录留在 `~/.claude`，下次可通过 `＋` 列表的「继续上次会话」用 `--resume` 接回。

> **2026-08-02：「子进程」这个前提不完全成立，得额外收尾。**
> Claude Code 2.1 自带守护进程（`claude daemon run` + `--bg-pty-host`），会话跑着
> 跑着会被交给它，之后父进程是那个 daemon 不是岛 —— 岛发的 SIGTERM 打在一个
> 已经不相干的 pid 上。实机抓到过四个：岛退了一个半小时还活着。
>
> 于是加了 `SessionReaper`：退 app 时按 `--settings <岛自己那份>` 扫一遍，
> 关单个 tab 时按 `--session-id <UUID>` 扫。先 SIGTERM 留时间落盘，还赖着的补
> SIGKILL。用户自己在终端里跑的 `claude` 不带这两个参数，误伤不了。
> 用户的原话：「如果是会保留，那就要改成关掉岛就要退出进程，不然跟弹框显示的
> 信息不一致」。

守护进程模式（任务在 app 之外存活、重开可重新接管）作为后续的设置项，接口已在 `SessionKit` 预留。**注意这跟上面那段是两回事**：上面是「别让它偷偷活下来」，这里是「让它有意地活下来，并且岛能重新接管、界面上说得清楚」。
...
> **这两条 2026-08-02 落地了**（`IslandModel.endNote` / `StatusBand.line` /
> `TabStrip.degradedHint`），落地时各补了一点原设计没写的：
>
> - 退出码 **128 以上换算成信号号**（143 → 「被终止（信号 15）」）。`SessionStatus`
>   就是按 `128 + 信号` 编的，把 143 原样甩给用户，他会去查自己的代码。
> - 「重启」按钮**复用**已有的「继续上次会话」（`resumeTab` → `--resume`）——
>   两种结束方式下会话记录都在 `~/.claude`，接法完全一样，只是文案不同。
> - 降级不只改状态带那行字，tab 条上还挂一个小闪电。只改文案的话，用户看到
>   「运行中（无详情）」也不知道该去修什么。
...
### 11.4 AX 逐帧驱动的开销 —— 已验证，可以逐帧跟

2026-08-06。要回答的是「拖拽岛调尺寸时，贴附的窗口能不能跟手」。
探针 `AXProbe.app`，靶子是 ChatGPT（`com.openai.codex`，**不是 `com.openai.chat`**）。

| 单次调用（中位数） | 空闲 | 目标 app 正在吐字 |
|---|---|---|
| 只设 `AXPosition` | 0.24 ms | 0.30 ms |
| 只设 `AXSize` | 3.71 ms | 8.20 ms |
| 两个都设 | 4.19 ms | 4.76 ms |

**60fps 灌 3 秒实测 59.99fps，松手时窗口离命令值差 0.09px、0.26ms 追平 —— 没有拖尾。**
AX 的 set 是同步落地的，不是异步追赶。120Hz 也跑得动（119.96fps），60fps 有余量。
目标 app 忙不忙不影响结论。

**结论：直接逐帧跟，不需要「拖拽中只挪位置、松手补尺寸」那类降级。** 但两条是硬的：

1. **AX 调用必须在后台队列。** p95 在 6.6–9.7 ms，偶发 15–25 ms，而 60fps 的预算是
   16.7 ms；`AXSize` 还撞到过一次 **520 ms**（空闲、忙时各一次）。放主线程上，
   一次就是岛自己卡半秒 —— 用户拖着边框，岛不动了
2. **拖拽时只保留最新目标 rect，积压的丢掉**，别排队

**最小尺寸只能「设完读回」，问不出来。** 二分压出 480×600（与 11.3 一致），但窗口
只暴露 `AXPosition` / `AXSize` 两个可写属性，**`AXMinValue` 根本不存在**。所以别去
二分探测（几十次调用），把内容区 rect 发下去、读回实际值，岛反过来拿这个实际值
钉住自己的拖拽下限。一次调用，且自动适配任意 app。
