...
模式芯片显示当前档位与快捷键提示 `⇧⇥`，点击或按 ⇧Tab 在 `默认 → Plan → 自动接受 → 跳过确认` 之间轮换，非默认档位用蓝色标出 —— 用户得知道自己现在在什么模式下按回车。

> 第 2 阶段接真实数据：额度三项走 `/usage` 或 statusline，模式与子代理走 hook，填进 `SessionUsage`。⇧Tab 届时应原样喂给 PTY、由 Claude Code 自己切，岛只反映结果。

**拖拽调整尺寸**：展开态底边是拖拽手柄 —— 中间段改高度，两个下角同时改宽高（岛相对屏幕中线对称，所以左右边缘每移 1pt，总宽变 2pt）。底边中央画一条可见的短横条，否则用户不知道这里能拖。宽度可拖范围 420pt 到「屏幕宽 − 160」，内容区高度 160pt 到「屏幕高 × 0.85 − 固定开销」。承载岛的 `NSPanel` 必须跟着一起长大，否则岛会被窗口边界切掉。
